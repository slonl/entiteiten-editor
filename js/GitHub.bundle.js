...
var Repository = function (_Requestable) {
   ...
   _createClass(Repository, [{
      ...
      key: 'getShaByTree',
      value: function getShaByTree(branch, path, cb) {
         var filename = path.split("/").pop();
         path = path.split("/");
         path.pop();
         path = path.join("/");

         return this._request('GET', '/repos/' + this.__fullname + '/git/trees/' + branch + ":" + escape(path), null, cb)
         .then(function(result) {
             var sha = result.data.tree.filter(function(entry) {
      ...
      key: 'getShaByCommits',
      value: function getShaByCommits(branch, path, cb) {
         branch = branch ? '&sha=' + branch : '';
         return this._request('GET', '/repos/' + this.__fullname + '/commits?path=/' + path + branch, null, cb);
      //   return this._request('GET', '/repos/' + this.__fullname + '/contents/' + path + branch, null, cb);
      }
      ...
      value: function writeFile(branch, path, content, message, options, cb) {
         ...
         return this.getShaByTree(branch, filePath).then(function (response) {
...
function callbackErrorOrThrow(cb, path) {
   return function handler(object) {
      ...
      if (object.hasOwnProperty('config')) {
         ...
         if (typeof _object$response !== "undefined") {
             var status = _object$response.status,
            ...
            var message = "Unknown status error making request";
            error = new ResponseError(message, path, object);
            log(message + ' ' + JSON.stringify(object.data));
         }
      } else {
